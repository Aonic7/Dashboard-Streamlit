    ...
    def plotting(self):
        ...
        if self.flag != True:
            try:
                ...
                # ax1.legend()
                # plt.show()
                df=pd.DataFrame({'y_actual':self.y_actual,'y_pred':self.y_pred})
                fig = plt.figure(figsize=(10, 4))
                sn.scatterplot(x='y_actual',y='y_pred',data=df,label='Real Prediction')
                sn.lineplot(x='y_actual',y='y_test',data=df,color='red',alpha=0.5,label='Optimal Prediction')
